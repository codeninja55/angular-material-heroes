...
Another topic of interest in [Type Assertion](#Type Assertion).
...
You will notice, we will be using the `let` keyword instead of JavaScript’s `var` keyword which you might be more familiar with. The `let` keyword is actually a newer JavaScript construct that TypeScript makes available. We’ll discuss the details later, but many common problems in JavaScript are alleviated by using `let`, so you should use it instead of `var` whenever possible.
...
x = "world"; // OK, 'string' can be assigned to 'string | number'

console.log(x.toString()); // OK, 'string' and 'number' both have 'toString'

x = true; // Error, 'boolean' isn't 'string | number'
```

Union types are an advanced topic that will be covered in [3.1.10. Advanced Types](./3.1.10.advanced_types.md). 



## Enum

A helpful addition to the standard set of datatypes from JavaScript is the `enum`. As in languages like Python 3, Java, and C#, an enum is a way of giving more friendly names to sets of numeric values.
